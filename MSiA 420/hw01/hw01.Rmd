...
Cross validation suggests the square root model performs better than the 
linear model.
...
plot(residuals1$x, residuals1$y, xlab = "X", ylab = "Residuals", main = "Residuals vs X for Model 1")

# create the second plot of residuals vs x for the second model
plot(residuals2$x, residuals2$y, xlab = "X", ylab = "Residuals", main = "Residuals vs X for Model 2")
```

The residual plots for the square root model seem more appropriate than the linear model because the residuals vary about half as much (3 vs 1.5). This agrees with out conclusions from questions 6 and 7 above.

Over all, neither model would pass the OLS assumptions. If we were to take this analysis further, we would need to find a model that better fits the OLS assumptions.